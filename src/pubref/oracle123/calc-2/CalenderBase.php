...
class CalenderBase{
	...
	public $lines;//lines fr file.
	
	
	public $OutMatrix=array();//all output data to display
   ...
   public function _uniord($c) {
	...
	public function CalenderBase(){
		...
		echo "fname=" . $fname. "; totlines=".count($this->lines) . "<br>";		
	}
	...
	public function main(){
		...
		if(!isset($this->lines)){
			$this->lines = file($this->DataFileName);
		}
	...
	public function workline($line){
		$line2chaArr = $this->mb_str_split($line);
		$line2intArr=array();
		foreach($line2chaArr as $k=>$cha){
			$line2intArr[]=$this->_uniord($cha);
		}
		...
		if( ($idxMon > 0)  && ($idxJz > 0) ) {
			...
			$this->stat_tabular($idxMon, $idxJz, $line, $line2intArr);
			//echo $line."<br>";
		}
		else if($idxMon>0) {
			$this->OutMatrix[0][$idxMon] +=1; //only for mon
			$this->stat_month($idxMon, $line, $line2intArr);
			//echo $line."<br>";
		}

		else if($idxJz>0){
			$this->OutMatrix[$idxJz][0] += 1; //only for Jz date
			$this->stat_jz($idxJz, $line, $line2intArr);
		}
		else{
			//$this->OutMatrix[0][0] +=1;
			$this->stat_outof_tablular(0,0,$line, $line2intArr);
		}

		return;
	}
	public function stat_jz($idxJz, $line, $line2intArr){
	}
	public function stat_month($iMonth, $line, $line2intArr){
	}
	public function stat_tabular($row,$col, $line, $line2intArr){
	}
	public function stat_outof_tablular($row,$col, $line, $line2intArr){
	}
	...
	public function showMatrix_trs_Tm_rows($title, $caption, & $totAllMon, & $tot_bottom){
		//// tot with date
		...
		$totCount=0;
		$trs3 =  "<tr class='Total_for_month'><td title='$title'>$caption</td><td>Tm</td>";
		for($j=1; $j<=count($this->MONTHS); $j+=1){
			$t1 = $tot_bottom[$j];
...
class CalenderTarget extends CalenderBase{
	public $targetChar;
	public $targetIntval; //for chant char.
	public $statMatrix;
	public $tot_in_mon=Array();
	public $tot_in_day=Array();
	public function GetTargetCharIntval(){
		if( isset($this->targetIntval) ){
			return $this->targetIntval;
	    }
	}
	public function GetTargetChar(){
		if( isset($this->targetChar) ){
			return $this->targetChar;
	    }
		else{
			if(isset($this->targetIntval)){
				$ChantChar = "<font face='chant'>";
				$ChantChar .= html_entity_decode('&#'.$this->targetIntval.';', 0, 'UTF-8');
				$ChantChar .="</font>";
				return $ChantChar;
			}
		}
	...
	public function CalenderTarget(){
		parent::CalenderBase();
	}
	public function SetTarget($target){	
		...
		else if($nsize>=7){//1037794
			$this->targetIntval=$target;
		}
		else{
			die("<font color='red'>size=$nsize,target=$target</font> <== incorrect input");
		}
	...
	public function stat_jz($idxJz,$line,$line2intArr){
		if (strpos($line, $this->targetChar) === false) {
			//echo $line. "<br/>";
			if( false === in_array($this->targetIntval, $line2intArr)){
				return;
			}
		}
	...
	public function stat_tabular($iMonth, $idxJz, $line,$line2intArr){
		if (strpos($line, $this->targetChar) === false) {
			if( false === in_array($this->targetIntval, $line2intArr)){
				return;
			}
	...
	public function showWork(){
		...
		$this->showMatrix( $this->GetTargetChar() );
		return;
	}
}//CaenderBase
class CalenderTarget_Topic extends CalenderTarget{
	...
	public function showMatrix($caption){
        ...
        $ar=$this->showMatrix_trs_Tm_rows($this->GetTargetCharIntval(), $caption,  $totAllMon,  $tot_bottom);
		$this->trs_Tm = $ar[2];//Tm only.  
	}
...
//withGui in the line.
class CalenderTarget_Topic_withGui extends CalenderTarget_Topic{
	public function withGui($line2intArr){
		return in_array(1041536, $line2intArr) ; //with Gui=1041536
	}
	
	public function stat_jz($idxJz,$line,$line2intArr){
		if( false === in_array(1041536, $line2intArr) ) {
			echo "_";
			return;
		}
...
class AllWordsList extends CalenderBase{
	...
	public function workline($line){
		...
		foreach($arr as $i => $ch){
			...
			//////
			$intval =  $this->_uniord($ch); 
			if($intval>=1036288){
				if(!isset($this->KeyOracleArr[$ch])){
					$this->KeyOracleArr[$ch]=0;
				}
				$this->KeyOracleArr[$ch] +=1;
			}
	...
	public function showWork(){
		...
		foreach($this->KeyPhrase as $key=>$val){
			...
			echo $i . ":::($intval) " . $key ." --->$val <br>";
			$i+=1;
		}
		
		/////////////////////////
		//show oracle char
		echo "<hr/>KeyOracleArr<hr/>";
		echo "<table border='1'><tr><td>#</td><td>unicode</td><td>ch</td><td>Freq</td><td></td></tr>\n";
		arsort($this->KeyOracleArr);
		$i=0;
		$data2file="";
		foreach($this->KeyOracleArr as $key=>$val){
			$intval =  $this->_uniord($key); // ord($key); //str.charCodeAt(0);  
			$data2file .=$intval . "\n";
			//convert intval into unicode in html.
			$unicd = html_entity_decode('&#'.$intval.';', 0, 'UTF-8');
			echo "<tr><td>$i</td><td>$intval</td><td>$key</td><td>$val</td><td>$unicd</td></tr>\n";
			$i+=1;
		}
		echo "</table>";
		
		$file2save="../calendar_srcripts/Months_All_Chant_list.txt";
		$ret = file_put_contents($file2save,$data2file);
		echo $file2save . "...$ret";
	}
}
